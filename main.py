...
def inline(inlineQuery):
    ...
    results = []
    for i in resultsT:
        results.append(types.InlineQueryResultArticle(id=i, title=i, input_message_content=types.InputTextMessageContent(disable_web_page_preview=False, message_text=f"{i}\n{resultsL[c]}")))
        c+=1
